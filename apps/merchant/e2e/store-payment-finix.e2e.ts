...
test('Finix: declined payment — 402 from payment-result does not show CONFIRMED', async ({ page, placeOrder }) => {
  ...
  // Declined payment shows the declined sub-panel inside the PAYING state
  await expect(page.locator('#paying-declined')).toBeVisible({ timeout: 8_000 })
  await expect(page.locator('#paying-processing')).toBeHidden()
  await expect(page.locator('#paying-retry-btn')).toBeVisible()
  await expect(page.locator('#paying-back-btn')).toBeVisible()
})
