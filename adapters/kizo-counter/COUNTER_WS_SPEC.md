# Kizo Counter WebSocket API Specification

**Version:** 1.0
**Transport:** WebSocket (`ws://` on LAN, `wss://` if TLS terminated upstream)
**Endpoint:** `/counter`
**Implemented by:** Kizo Bun server
**Consumed by:** Kizo Counter Android app (Lenovo Tab One, counter tablet)
...
```
ws://<kizo-host>:<port>/counter?token=<api_token>
```

- `token` — optional bearer token for authentication. Kizo should reject connections with an invalid or missing token with HTTP 401 before the WebSocket handshake completes.
- The counter app reconnects automatically with exponential backoff (1s → 2s → 4s … max 30s) on any disconnect or failure.
- Kizo should support **one active counter connection per terminal** (identified by token or a `terminalId` query param if you need multiple counters).

### On connect

Kizo **must** immediately send a `config` message so the counter app knows the restaurant name and Finix credentials before any payment request arrives.
...
Sent when the cashier initiates a payment on Kizo. The counter app transitions from idle to the tip selection screen immediately on receipt.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `orderId` | string | yes | Kizo order ID, echoed back in the result |
...
## Messages: Counter → Kizo

### `counter_status`

Sent on WebSocket connect (after Kizo sends `config`) and whenever the D135 terminal connection state changes.
...
Kizo should surface this to the cashier UI (e.g. "Counter terminal offline" warning) to prevent sending payment requests to a disconnected terminal.

---

### `payment_result`

Sent **immediately** when the transaction resolves — before the customer dismisses the result screen. Kizo should unblock as soon as this arrives.
...
**Kizo's responsibilities on receipt:**
...
Sent **after** `payment_result` if the customer opts into an email receipt on the result screen. May arrive seconds after `payment_result`. Kizo should handle it independently.
...
| Counter never responds | Kizo should implement a timeout (suggested: 3 minutes) and allow the cashier to cancel |
